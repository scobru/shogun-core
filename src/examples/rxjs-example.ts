...
const runRxJSExample = async () => {
  ...
  const userProfile$ = shogun.observe<{ name: string; bio: string }>(
    "users/profile/123",
  );
  ...
  setTimeout(() => {
    ...
    shogun
      .rxPut<{ name: string; bio: string }>("users/profile/123", {
        name: "John Doe",
  ...
  const completedTodos$ = shogun
    .match<{ id: string; task: string; completed: boolean }>("todos")
    .pipe(
  ...
  const todoStats$ = shogun.compute<
    any,
  ...
  if (shogun.isLoggedIn()) {
    const userPreferences$ = shogun.observeUser<{
      theme: string;
    ...
    shogun
      .rxUserPut<{ theme: string; notifications: boolean }>("preferences", {
        theme: "dark",
        notifications: true,
      })
...
const chatAppExample = async () => {
  ...
  const usersSubscription = activeUsers$
    .pipe(
      ...
      map((users) =>
        users.filter(
          (user) => user.lastSeen && Date.now() - user.lastSeen < 5 * 60 * 1000,
        ),
      ),
    )
    .subscribe({
  ...
  shogun.rx
    .put<{ id: string; name: string; lastSeen: number }>(
      `rooms/${roomId}/users/${userPub}`,
      {
        id: userPub,
