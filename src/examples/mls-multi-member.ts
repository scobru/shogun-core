...
async function testMLSWorkingMultiMembers() {
  ...
  const managers = new Map<string, MLSManager>();

  try {
    // Step 1: Initialize all managers
    console.log("\n📋 Step 1: Initializing all managers");
    for (const memberId of members) {
      const manager = new MLSManager(memberId);
    ...
    const keyPackages = otherMembers.map(
      (id) => managers.get(id)!.getKeyPackage()!,
    );

    console.log(
      `➕ Adding ${otherMembers.length} members: ${otherMembers.join(", ")}`,
    );
    ...
    for (const senderId of members) {
      ...
      if (successCount === members.length) {
        console.log(
          `🎉 All ${members.length} members successfully decrypted ${senderId}'s message`,
        );
      } else {
        console.log(
          `⚠️ Only ${successCount}/${members.length} members could decrypt ${senderId}'s message`,
        );
      }
    }
    ...
    for (const [memberId, manager] of managers) {
      ...
      try {
        ...
        console.error(
          `❌ ${memberId} failed to decrypt after rotation:`,
          error,
        );
      }
    }
