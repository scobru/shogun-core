...
async function shogunCoreExample() {
  ...
  console.log(
    "⏳ This may take up to 30 seconds if peers are unreachable...\n",
  );
  ...
  const canLogin =
    signupResult?.success || signupResult?.error?.includes("already");
  ...
  if (canLogin) {
    ...
    try {
      console.log(
        "⏳ Login may take up to 30 seconds if peers are unreachable...",
      );
