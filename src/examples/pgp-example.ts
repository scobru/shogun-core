...
async function simplePGPExample() {
  try {
    ...
    // Generate key pairs
    const aliceKeys = await generatePGPKeyPair(
      "Alice",
      "alice@example.com",
      "alice123",
    );
    const bobKeys = await generatePGPKeyPair(
      "Bob",
      "bob@example.com",
      "bob123",
    );
    ...
    const encrypted = await encryptPGPMessage(message, bobKeys.publicKey);
...
async function advancedPGPExample() {
  let manager: PGPManager | undefined;
  ...
  try {
    ...
    manager = await createPGPManager();
    ...
    const groupMessage =
      "Hello everyone! This is a group message from Alice. 👥";

    // Encrypt for Bob
    const encryptedForBob = await manager.encryptMessage(
      groupMessage,
      userKeys.get("Bob").publicKey,
      userKeys.get("Alice").privateKey, // Alice signs
      "alice123",
    );

    // Encrypt for Charlie
    const encryptedForCharlie = await manager.encryptMessage(
      groupMessage,
      userKeys.get("Charlie").publicKey,
      userKeys.get("Alice").privateKey, // Alice signs
      "alice123",
    );
    ...
    // Bob decrypts
    const bobDecrypted = await manager.decryptMessage(
      encryptedForBob.message,
      userKeys.get("Bob").privateKey,
      "bob123",
    );

    // Charlie decrypts
    const charlieDecrypted = await manager.decryptMessage(
      encryptedForCharlie.message,
      userKeys.get("Charlie").privateKey,
      "charlie123",
    );
    ...
    const aliceKeyArmored = await manager.exportKey(
      userKeys.get("Alice").publicKey,
      "armored",
    );
    const aliceKeyBinary = await manager.exportKey(
      userKeys.get("Alice").publicKey,
      "binary",
    );

    console.log("✅ Alice's key exported in armored and binary formats");
    console.log("Armored length:", (aliceKeyArmored as string).length);
    console.log("Binary length:", (aliceKeyBinary as Uint8Array).length);
    ...
    console.log(
      "Imported key matches original:",
      importedKey === userKeys.get("Alice").publicKey,
    );
...
async function runAllPGPExamples() {
  ...
  const demoResult = await demonstratePGP();

  console.log("\n📊 Final Results:");
  console.log("Simple PGP:", simpleResult.success ? "✅ PASSED" : "❌ FAILED");
  console.log(
    "Advanced PGP:",
    advancedResult.success ? "✅ PASSED" : "❌ FAILED",
  );
  console.log("Full Demo:", demoResult.success ? "✅ PASSED" : "❌ FAILED");

  const allPassed =
    simpleResult.success && advancedResult.success && demoResult.success;
