...
const validationUtils = {
  ...
  hasRequiredProperties: (
    ...
    if (!validationUtils.isObject(obj)) return false;
  ...
  validateObject: (
    ...
    for (const [prop, validator] of Object.entries(schema)) {
      if (!(prop in obj) || !validator(obj[prop])) return false;
    }
...
describe("ValidationUtils", () => {
  describe("isValidEmail", () => {
    test("dovrebbe validare correttamente gli indirizzi email", () => {
      ...
      expect(validationUtils.isValidEmail("user.name@example.co.uk")).toBe(
        true,
      );
  ...
  describe("hasRequiredProperties", () => {
    test("dovrebbe verificare correttamente la presenza di proprietà richieste", () => {
      ...
      expect(validationUtils.hasRequiredProperties(obj, ["a", "b", "c"])).toBe(
        true,
      );

      expect(validationUtils.hasRequiredProperties(obj, ["d"])).toBe(false);
      expect(validationUtils.hasRequiredProperties(obj, ["a", "d"])).toBe(
        false,
      );
      expect(
        validationUtils.hasRequiredProperties("not an object", ["a"]),
      ).toBe(false);
    });
  });
  ...
  describe("validateObject", () => {
    test("dovrebbe validare correttamente gli oggetti secondo uno schema", () => {
      ...
      expect(validationUtils.validateObject(missingPropObj, schema)).toBe(
        false,
      );

      expect(validationUtils.validateObject("not an object", schema)).toBe(
        false,
      );
    });
  });
