...
class ShogunError extends Error {
  ...
  constructor(message: string, errorType: ErrorType = ErrorType.General) {
    super(message);
...
class ErrorHandler {
  ...
  private errorHistory: ShogunError[] = [];
  private listeners: Array<(error: ShogunError) => void> = [];
  private readonly maxErrorHistorySize = 10;
  ...
  createError(message: string, errorType: ErrorType = ErrorType.General): ShogunError {
    return new ShogunError(message, errorType);
  }
  
  handleError(error: ShogunError): void {
  ...
  getRecentErrors(): ShogunError[] {
    return [...this.errorHistory];
  }
  
  addListener(listener: (error: ShogunError) => void): void {
    this.listeners.push(listener);
  }
  
  removeListener(listener: (error: ShogunError) => void): void {
  ...
  notifyListeners(error: ShogunError): void {
  ...
  getErrorsByType(errorType: ErrorType): ShogunError[] {
