...
const cryptoUtils = {
  ...
  encryptData: async (
    ...
    return new Uint8Array(
      dataBytes.map((byte, index) => byte ^ key[index % key.length]),
    );
  },
  ...
  decryptData: async (
    ...
    return new Uint8Array(
      encryptedData.map((byte, index) => byte ^ key[index % key.length]),
    );
  },
  ...
  deriveKeyFromPassword: async (
    ...
    return new Uint8Array(
      [...passwordBytes, ...salt].map((b) => (b * 2) % 256),
    );
  },
};
...
describe("CryptoUtils", () => {
  ...
  test("dovrebbe derivare chiavi consistenti da password", async () => {
    ...
    const differentKey = await cryptoUtils.deriveKeyFromPassword(
      "different-password",
      salt,
    );
