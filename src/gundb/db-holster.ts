...
class DataBaseHolster {
  ...
  constructor(holster: any, core?: any, sea?: any) {
    ...
    if (typeof holster.user !== 'function') {
      throw new Error(
        'Holster instance is invalid: holster.user is not a function',
      );
    }
    ...
    if (!this.sea) {
      ...
      } else if ((globalThis as any).Holster?.SEA) {
        this.sea = (globalThis as any).Holster.SEA;
    ...
    return new Promise<AuthResult>(async (resolve) => {
      ...
      try {
        ...
        if (!aliasAvailable) {
          ...
          const existingPub = await new Promise<string | null>((resolve) => {
            this.usernamesNode.next(normalizedUsername, null, (pub: any) => {
              resolve(pub || null);
            });
          });
   ...
   * @internal
   */
  private async createUserWithPair(
    ...
    try {
      ...
      // Put user data
      this.holster.wire.put(graph, (err: any) => {
        if (err) {
          resolve({
            success: false,
        ...
        this.holster.wire.put(relGraph, async (relErr: any) => {
          if (relErr) {
            resolve({
              success: false,
   ...
   */
  rx(): RxJSHolster {
    return this._rxjs as RxJSHolster;
  }
   ...
   */
  private createGunProxy(): any {
    const self = this;
    const createChainProxy = (chain: any): any => {
      return {
        get: (nextKey: string) => {
