...
exports.ChallengePlugin = void 0;
const base_1 = require("../base");
const shogun_1 = require("../../interfaces/shogun");
class ChallengePlugin extends base_1.BasePlugin {
    constructor() {
        super(...arguments);
        this.name = 'challenge';
        this.version = '1.0.0';
        this.description = 'Challenge-response authentication using Gun SEA signatures';
     ...
     */
    async requestChallenge(serverUrl, username, password) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(err.error || `Challenge request failed: ${response.status}`);
            }
     ...
     */
    async signChallenge(challenge, pair) {
     ...
     */
    async verifyChallenge(signedChallenge, pubKey, expectedChallenge) {
        const sea = this.getSEA();
        if (!sea || !sea.verify) {
     ...
     */
    async login(serverUrl, username, password, pair) {
        try {
            // Step 1: Request challenge
            const challengeData = await this.requestChallenge(serverUrl, username, password);
            // Step 2: Sign challenge locally
            const signedChallenge = await this.signChallenge(challengeData.challenge, pair);
...
exports.ChallengePlugin = ChallengePlugin;
