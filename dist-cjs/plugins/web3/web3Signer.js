...
class Web3Signer {
    ...
    async createSigningCredential(address) {
        try {
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.WALLET, 'WEB3_CREDENTIAL_ERROR', 'Error creating Web3 signing credential', error);
    ...
    async requestSignature(address) {
        try {
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.WALLET, 'WEB3_SIGNATURE_ERROR', 'Failed to request signature', error);
            throw error;
        }
    }
    /**
    ...
    createAuthenticator(address) {
        ...
        return async (data) => {
            try {
                ...
                errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.AUTHENTICATION, 'WEB3_AUTH_ERROR', 'Web3 authentication error', error);
                throw error;
            }
        };
    }
        ...
        try {
            ...
            return new Promise((resolve) => {
                ...
                gunInstance.user().auth(derivedPair, (authAck) => {
                    if (authAck.err) {
                        errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.AUTHENTICATION, 'WEB3_GUN_AUTH_FAILED', 'Web3Signer - GunDB auth failed', authAck.err);
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.AUTHENTICATION, 'WEB3_AUTH_EXISTING_FAILED', 'Error authenticating with deterministic pair', error);
    ...
    async createDerivedKeyPairFromAddress(address, extra) {
        try {
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.ENCRYPTION, 'WEB3_KEY_DERIVATION_ERROR', 'Error creating derived key pair from address', error);
            throw error;
        }
    }
    /**
        ...
        try {
            ...
            return new Promise((resolve) => {
                ...
                gunInstance.user().create(derivedPair, (ack) => {
                    if (ack.err) {
                        errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.DATABASE, 'WEB3_USER_CREATION_FAILED', 'Web3Signer - user creation failed, trying auth', ack.err);
                        // Try to login if user already exists
                        gunInstance.user().auth(derivedPair, (authAck) => {
                            if (authAck.err) {
                                errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.AUTHENTICATION, 'WEB3_USER_AUTH_FAILED', 'Web3Signer - auth also failed', authAck.err);
                                resolve({ success: false, error: authAck.err });
                            }
                            else {
                                const userPub = authAck.pub;
                        ...
                        gunInstance.user().auth(derivedPair, (authAck) => {
                            if (authAck.err) {
                                errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.AUTHENTICATION, 'WEB3_LOGIN_AFTER_CREATION_FAILED', 'Web3Signer - login after creation failed', authAck.err);
                                resolve({ success: false, error: authAck.err });
                            }
                            else {
                                const userPub = authAck.pub;
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.DATABASE, 'WEB3_CREATE_USER_ERROR', 'Error creating Gun user', error);
            return { success: false, error: error.message };
        }
    }
    /**
    ...
    async signWithDerivedKeys(data, address, extra) {
        try {
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.SIGNATURE, 'WEB3_DERIVED_SIGN_ERROR', 'Error signing with derived keys', error);
            throw error;
        }
    }
    /**
    ...
    async getGunUserPub(address) {
        try {
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.ENCRYPTION, 'WEB3_GET_PUB_ERROR', 'Error getting Gun user pub', error);
            return undefined;
        }
    }
    /**
    ...
    getPassword(address) {
        try {
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.ENCRYPTION, 'WEB3_GET_PASSWORD_ERROR', 'Error getting password', error);
            return undefined;
        }
    }
    /**
    ...
    async verifyConsistency(address, expectedUserPub) {
        try {
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.ENCRYPTION, 'WEB3_CONSISTENCY_ERROR', 'Error verifying consistency', error);
