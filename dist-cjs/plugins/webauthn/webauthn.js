...
class Webauthn extends eventEmitter_1.EventEmitter {
    ...
    async sign(data) {
        ...
        const dataBytes = ethers_1.ethers.toUtf8Bytes(dataString);
        const hash = ethers_1.ethers.sha256(dataBytes);
        const challenge = ethers_1.ethers.getBytes(hash);
        const options = {
            challenge: challenge,
            rpId: this.config.rpId,
            userVerification: this.config.userVerification,
...
async function deriveWebauthnKeys(username, credentialIdOrSeedPhrase, useSeedPhrase = false) {
    if (useSeedPhrase) {
        ...
        const { password, seed } = await (0, seedPhrase_1.deriveCredentialsFromMnemonic)(credentialIdOrSeedPhrase, username);
