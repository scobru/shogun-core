...
class AuthManager {
    ...
    async signUp(username, password, pair) {
        try {
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.AUTHENTICATION, 'SIGNUP_FAILED', `Error during registration for user ${username}: ${error instanceof Error ? error.message : String(error)}`, error);
            this.core.emit('debug', {
                action: 'signup_error',
                error: error instanceof Error ? error.message : String(error),
                username,
