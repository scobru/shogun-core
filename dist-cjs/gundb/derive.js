...
async function default_1(pwd, extra, options = {}) {
    ...
    if (includeP256) {
        ...
        const [signingKeys, encryptionKeys] = await Promise.all(salts.map(async ({ label }) => {
            ...
            if (!nist_js_1.p256.utils.isValidPrivateKey(privateKey)) {
                throw new Error(`Invalid private key for ${label}`);
            }
            const publicKey = nist_js_1.p256.getPublicKey(privateKey, false);
            return {
    ...
    if (includeSecp256k1Bitcoin) {
        ...
        if (!secp256k1_js_1.secp256k1.utils.isValidPrivateKey(bitcoinPrivateKey)) {
            throw new Error('Invalid secp256k1 private key for Bitcoin');
        }
        const bitcoinPublicKey = secp256k1_js_1.secp256k1.getPublicKey(bitcoinPrivateKey, true); // Compressed
    ...
    if (includeSecp256k1Ethereum) {
        ...
        if (!secp256k1_js_1.secp256k1.utils.isValidPrivateKey(ethereumPrivateKey)) {
            throw new Error('Invalid secp256k1 private key for Ethereum');
        }
        const ethereumPublicKey = secp256k1_js_1.secp256k1.getPublicKey(ethereumPrivateKey, false); // Uncompressed
...
async function stretchKey(input, salt, iterations = 300000) {
    try {
        ...
        const fallbackKey = (0, pbkdf2_js_1.pbkdf2)(sha2_js_1.sha256, inputBytes, salt, {
            c: iterations,
            dkLen: 32,
        });
...
function deriveP2PKHAddress(publicKey) {
    ...
    const sha256Hash = (0, sha2_js_1.sha256)(publicKey);
    // 2. RIPEMD160 hash del risultato
    const ripemd160Hash = (0, legacy_js_1.ripemd160)(sha256Hash);
