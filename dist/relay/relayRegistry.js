...
class RelayRegistry {
    ...
    constructor(config, shogun, signer) {
        ...
        try {
            ...
            if (this.signer && this.provider) {
                ...
                (0, logger_1.log)(`RelayRegistry initialized with signer at ${this.registryAddress}`);
            }
            else if (this.provider) {
                this.registryContract = new ethers_1.ethers.Contract(this.registryAddress, RELAY_REGISTRY_ABI, this.provider);
                (0, logger_1.log)(`RelayRegistry initialized in read-only mode at ${this.registryAddress}`);
            }
        }
        catch (error) {
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "RELAY_REGISTRY_INIT_FAILED", "Failed to initialize RelayRegistry", error);
        }
    }
    /**
    ...
    async getRelayCount() {
        try {
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "GET_RELAY_COUNT_FAILED", "Failed to get relay count", error);
            return 0;
        }
    }
    /**
     ...
     */
    async isRegistered(relayAddress) {
        try {
            if (!this.registryContract) {
                throw new Error("Registry contract not initialized");
            }
            if (!ethers_1.ethers.isAddress(relayAddress)) {
                throw new Error("Invalid relay address format");
            }
            return await this.registryContract.isRegistered(relayAddress);
        }
        catch (error) {
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "IS_REGISTERED_CHECK_FAILED", `Failed to check if ${relayAddress} is registered`, error);
            return false;
        }
    }
    /**
    ...
    async getProtocolConfig() {
        try {
            ...
            const [protocolPrice, feePercentage, minStake, totalStaked] = await Promise.all([
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "GET_PROTOCOL_CONFIG_FAILED", "Failed to get protocol configuration", error);
            return null;
        }
    }
    /**
     ...
     * @returns Transaction response or null if failed
     */
    async registerRelayContract(relayOwner, url, stake) {
        try {
            if (!this.registryContract || !this.signer) {
                throw new Error("Registry contract not initialized or no signer available");
            }
            if (!ethers_1.ethers.isAddress(relayOwner)) {
                throw new Error("Invalid owner address format");
            }
            return await this.registryContract.registerRelayContract(relayOwner, url, stake);
        }
        catch (error) {
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "REGISTER_RELAY_FAILED", "Failed to register relay contract", error);
            return null;
        }
    }
    /**
     ...
     * @returns Transaction response or null if failed
     */
    async updateRelayStake(relayContract, newStake) {
        try {
            if (!this.registryContract || !this.signer) {
                throw new Error("Registry contract not initialized or no signer available");
            }
            if (!ethers_1.ethers.isAddress(relayContract)) {
                throw new Error("Invalid relay address format");
            }
            return await this.registryContract.updateRelayStake(relayContract, newStake);
        }
        catch (error) {
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "UPDATE_STAKE_FAILED", `Failed to update stake for relay ${relayContract}`, error);
            return null;
        }
    }
    /**
     ...
     * @returns Transaction response or null if failed
     */
    async unregisterRelayContract(relayContractAddress) {
        try {
            if (!this.registryContract || !this.signer) {
                throw new Error("Registry contract not initialized or no signer available");
            }
            if (!ethers_1.ethers.isAddress(relayContractAddress)) {
                throw new Error("Invalid relay address format");
            }
            return await this.registryContract.unregisterRelayContract(relayContractAddress);
        }
        catch (error) {
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "UNREGISTER_RELAY_FAILED", `Failed to unregister relay ${relayContractAddress}`, error);
            return null;
        }
    }
    /**
     ...
     * @returns Transaction response or null if failed
     */
    async updateRelayUrl(relayContractAddress, newUrl) {
        try {
            if (!this.registryContract || !this.signer) {
                throw new Error("Registry contract not initialized or no signer available");
            }
            ...
            return await this.registryContract.updateRelayUrl(relayContractAddress, newUrl);
        }
        catch (error) {
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "UPDATE_URL_FAILED", `Failed to update URL for relay ${relayContractAddress}`, error);
            return null;
        }
    }
    /**
     ...
     * @returns Transaction response or null if failed
     */
    async setProtocolPrice(newPrice) {
        try {
            if (!this.registryContract || !this.signer) {
                throw new Error("Registry contract not initialized or no signer available");
            }
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "SET_PROTOCOL_PRICE_FAILED", "Failed to set protocol price", error);
            return null;
        }
    }
    /**
     ...
     * @returns Transaction response or null if failed
     */
    async setProtocolFeePercentage(newPercentage) {
        try {
            if (!this.registryContract || !this.signer) {
                throw new Error("Registry contract not initialized or no signer available");
            }
            ...
            return await this.registryContract.setProtocolFeePercentage(newPercentage);
        }
        catch (error) {
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "SET_FEE_PERCENTAGE_FAILED", "Failed to set protocol fee percentage", error);
            return null;
        }
    }
    /**
     ...
     * @returns Transaction response or null if failed
     */
    async setDistributionPeriod(newPeriod) {
        try {
            if (!this.registryContract || !this.signer) {
                throw new Error("Registry contract not initialized or no signer available");
            }
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "SET_DISTRIBUTION_PERIOD_FAILED", "Failed to set distribution period", error);
            return null;
        }
    }
    /**
     ...
     * @returns Transaction response or null if failed
     */
    async setMinimumStake(newMinStake) {
        try {
            if (!this.registryContract || !this.signer) {
                throw new Error("Registry contract not initialized or no signer available");
            }
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "SET_MINIMUM_STAKE_FAILED", "Failed to set minimum stake", error);
            return null;
        }
    }
    /**
     ...
     * @returns Transaction response or null if failed
     */
    async withdrawProtocolFees(amount) {
        try {
            if (!this.registryContract || !this.signer) {
                throw new Error("Registry contract not initialized or no signer available");
            }
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "WITHDRAW_FEES_FAILED", "Failed to withdraw protocol fees", error);
            return null;
        }
    }
    /**
    ...
    setSigner(signer) {
        try {
            ...
            if (this.provider) {
                ...
                this.registryContract = new ethers_1.ethers.Contract(this.registryAddress, RELAY_REGISTRY_ABI, this.signer);
                (0, logger_1.log)(`Updated signer for RelayRegistry`);
                return true;
            }
            return false;
        }
...
exports.RelayRegistry = RelayRegistry;
