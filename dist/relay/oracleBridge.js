...
class OracleBridge {
    ...
    constructor(config, shogun, signer) {
        ...
        try {
            ...
            if (this.signer && this.provider) {
                this.contract = new ethers_1.ethers.Contract(this.contractAddress, ORACLE_BRIDGE_ABI, this.signer);
                (0, logger_1.log)(`OracleBridge initialized with signer at ${this.contractAddress}`);
            }
            else if (this.provider) {
                this.contract = new ethers_1.ethers.Contract(this.contractAddress, ORACLE_BRIDGE_ABI, this.provider);
                (0, logger_1.log)(`OracleBridge initialized in read-only mode at ${this.contractAddress}`);
            }
        }
        catch (error) {
     ...
     */
    async verifyRoot(epochId, expectedRoot) {
        try {
     ...
     */
    async getRootTimestamp(epochId) {
        try {
            if (!this.contract) {
                throw new Error("Contract not initialized");
            }
            ...
            errorHandler_1.ErrorHandler.handle(errorHandler_1.ErrorType.CONTRACT, "GET_ROOT_TIMESTAMP_FAILED", `Failed to get timestamp for root of epoch ${epochId}`, error);
            return BigInt(0);
        }
    }
}
exports.OracleBridge = OracleBridge;
