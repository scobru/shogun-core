 ...
 */
export class DIDPlugin extends BasePlugin {
    constructor() {
        super(...arguments);
        this.name = "did";
        this.version = "1.0.0";
        this.description = "Provides Decentralized Identifiers (DID) functionality for ShogunCore";
        this.did = null;
    }
    /**
     * @inheritdoc
    ...
    async ensureUserHasDID(options) {
        try {
            ...
            ErrorHandler.handle(ErrorType.DID, "ENSURE_DID_FAILED", `Error ensuring user has DID: ${error instanceof Error ? error.message : String(error)}`, error);
            return null;
        }
    }
}
